...
const { data: overview, status: overviewStatus } = await useFetch("/api/seller/stats/overview", { lazy: true });
const { data: productsStock, status: productsStockStatus } = await useFetch<Array<{ name: string; stock: number }>>("/api/seller/stats/products-stock", { lazy: true });
const { data: productsRating, status: productsRatingStatus } = await useFetch<Array<{ name: string; rating: number }>>("/api/seller/stats/products-rating", { lazy: true });
const { data: ratingDistribution, status: ratingDistributionStatus } = await useFetch<{
  averageRating: number;
  totalReviews: number;
  distribution: Array<{ rating: number; count: number; percentage: number }>;
}>("/api/seller/stats/rating-distribution", { lazy: true });
const { data: ratingByProvince, status: ratingByProvinceStatus } = await useFetch<Array<{
  province: string;
  count: number;
  avgRating: number;
...
const loading = computed(() =>
  ...
  || productsStockStatus.value === "pending"
  || productsRatingStatus.value === "pending"
  || ratingDistributionStatus.value === "pending"
  || ratingByProvinceStatus.value === "pending",
);
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="card bg-base-100 shadow-md hover:shadow-xl transition-shadow">
              <div class="card-body">
                ...
                <NuxtLink
                  to="/api/seller/reports/products-stock"
                  target="_blank"
